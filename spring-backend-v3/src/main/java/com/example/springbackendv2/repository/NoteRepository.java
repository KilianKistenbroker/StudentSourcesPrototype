...
public interface NoteRepository extends JpaRepository<Note, Long> {
    ...
    @Query(value = "select n from Note n WHERE n.user_id = :userId", nativeQuery = true)
    List<Note> findAllByUserId(@Param("userId") Long userId);

    @Query(value = "select n from Note n WHERE n.user_id = :userId ODER BY id DESC LIMIT 1", nativeQuery = true)
    Note findLatestNoteByUserId(@Param("userId") Long userId);

    @Query(value = "update Note n set n.text = :text WHERE n.id = :id", nativeQuery = true)
